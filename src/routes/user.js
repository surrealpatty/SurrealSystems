...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const token = jwt.sign({ id: newUser.id, email: newUser.email }, signingSecret, {
        expiresIn: '1d',
      });
      ...
      if (err && err.name === 'SequelizeUniqueConstraintError') {
        const details = (err.errors || []).map((e) =>
          e && e.message ? e.message : e && e.path ? `${e.path} must be unique` : String(e),
        );
      ...
      if (process.env.NODE_ENV && process.env.NODE_ENV !== 'production') {
        const message =
          err && err.message ? `Registration failed: ${err.message}` : 'Registration failed';
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const signingSecret = getSigningSecret();
      if (!signingSecret) return sendError(res, 'Server misconfigured', 500);
      const token = jwt.sign({ id: userRec.id, email: userRec.email }, signingSecret, {
        expiresIn: '1d',
      });
